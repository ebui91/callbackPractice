...
// If the name does not exist, invoke the callback with false as an argument.
  ...
  //answer with ES6 syntax
  const contains=(names,name,cb)=>{names.indexOf(name)!==-1?cb(true):cb(false);}
...
// Invoke the callback with the modified array as an argument.
  ...
  //Code Here
  function uniq(names,cb){names=names.filter((current,index)=>names.indexOf(current)==index);
  return cb(names);}

  //ES6
  //const uniq(names,cb)=>{names=names.filter((current,index)=>names.indexOf(current)==index);return cb(names);}
